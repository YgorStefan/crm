    ...
    <a href="<?= APP_URL ?>/clients"
       class="bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-gray-100 dark:border-zinc-800 p-4 flex items-center gap-3 hover:shadow-md hover:border-indigo-200 dark:hover:border-indigo-700 transition-all">
        ...
        <div>
            ...
            <p class="text-sm text-gray-500 dark:text-zinc-400">Clientes ativos</p>
        </div>
    </a>
    ...
    <a href="<?= APP_URL ?>/tasks"
       class="bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-gray-100 dark:border-zinc-800 p-4 flex items-center gap-3 hover:shadow-md hover:border-amber-200 dark:hover:border-amber-700 transition-all">
        ...
        <div>
            ...
            <p class="text-sm text-gray-500 dark:text-zinc-400">Minhas tarefas abertas</p>
        </div>
    </a>
    ...
    <a href="<?= APP_URL ?>/tasks"
       class="bg-white dark:bg-zinc-900 rounded-xl shadow-sm border <?= count($overdueTasks) > 0 ? 'border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/20' : 'border-gray-100 dark:border-zinc-800 hover:border-red-200 dark:hover:border-red-700' ?> p-4 flex items-center gap-3 hover:shadow-md transition-all">
        ...
        <div>
            ...
            <p class="text-sm <?= count($overdueTasks) > 0 ? 'text-red-500 dark:text-red-400' : 'text-gray-500 dark:text-zinc-400' ?>">Tarefas atrasadas</p>
        </div>
    </a>
    ...
    <a href="<?= APP_URL ?>/pipeline"
       class="bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-gray-100 dark:border-zinc-800 p-4 flex items-center gap-3 hover:shadow-md hover:border-green-200 dark:hover:border-green-700 transition-all">
        ...
        <div>
            ...
            <p class="text-sm text-gray-500 dark:text-zinc-400">Negócios ganhos</p>
        </div>
    </a>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
    ...
    <div class="bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-gray-100 dark:border-zinc-800 overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-100 dark:border-zinc-800 flex justify-between items-center">
            <div class="flex items-center gap-2">
                ...
                <span class="font-semibold text-gray-700 dark:text-zinc-200">Próximas Tarefas (7 dias)</span>
            </div>
        ...
        <?php if (empty($upcomingTasks)): ?>
            <div class="px-5 py-8 text-center text-gray-400 dark:text-zinc-500 text-sm">Nenhuma tarefa nos próximos 7 dias 🎉</div>
        <?php else: ?>
            <div class="divide-y divide-gray-50 dark:divide-zinc-800 overflow-y-auto" style="max-height: 416px;">
                <?php
                ...
                foreach ($upcomingTasks as $task):
                    ...
                    <div class="px-5 py-3 flex items-center gap-3">
                        ...
                        <div class="flex-1 min-w-0">
                            <a href="<?= APP_URL ?>/tasks" class="text-sm font-medium text-gray-700 dark:text-zinc-200 hover:text-indigo-600 truncate block">
                                <?= htmlspecialchars($task['title'], ENT_QUOTES, 'UTF-8') ?></a>
                            <?php if ($task['client_name']): ?>
                                <p class="text-xs text-gray-400 dark:text-zinc-500 truncate">👥 <?= htmlspecialchars($task['client_name'], ENT_QUOTES, 'UTF-8') ?></p>
                            <?php endif; ?>
                        </div>
                        <span class="text-xs text-gray-400 dark:text-zinc-500 flex-shrink-0"><?= date('d/m', strtotime($task['due_date'])) ?></span>
                    </div>
                <?php endforeach; ?>
            </div>
        <?php endif; ?>
    ...
    <div class="bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-gray-100 dark:border-zinc-800 overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-100 dark:border-zinc-800 flex justify-between items-center">
            <div class="flex items-center gap-2">
                <span class="text-indigo-500 flex-shrink-0">🕐</span>
                <span class="font-semibold text-gray-700 dark:text-zinc-200">Atividade Recente</span>
            </div>
        ...
        if (empty($recentInteractions)):
            ?>
            <div class="px-5 py-8 text-center text-gray-400 dark:text-zinc-500 text-sm">Nenhuma interação registrada.</div>
        <?php else: ?>
            <div class="divide-y divide-gray-50 dark:divide-zinc-800 overflow-y-auto" style="max-height: 416px;">
                <?php foreach ($recentInteractions as $inter): ?>
                    <div class="px-5 py-3 flex items-start gap-3">
                        ...
                        <span class="text-xs text-gray-400 dark:text-zinc-500 flex-shrink-0">
...
<div class="mb-8">
    <div class="bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-gray-100 dark:border-zinc-800 p-5">
        <h4 class="text-sm font-semibold text-gray-600 dark:text-zinc-400 mb-4">Distribuição no Pipeline</h4>
