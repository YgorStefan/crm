...
<div class="max-w-6xl mx-auto">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <?php
            // Detecta "Venda Fechada" pelo nome da etapa — case-insensitive (D-04)
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden" id="cotas-section">
                ...
                <div id="cotas-list" class="divide-y divide-gray-50">
                    <?php if (empty($sales)): ?>
                    <p class="px-5 py-6 text-sm text-gray-400 text-center" id="cotas-empty">Nenhuma cota cadastrada ainda.</p>
                    <?php else: ?>
                    ...
                    <div class="px-5 py-4 flex items-start justify-between gap-3" data-sale-id="<?= $sale['id'] ?>">
                        <div class="grid grid-cols-2 gap-x-6 gap-y-1 text-sm flex-1">
                            <div><span class="text-gray-400 text-xs">Grupo:</span> <span class="text-gray-700 font-medium"><?= htmlspecialchars($sale['grupo'] ?? '—', ENT_QUOTES, 'UTF-8') ?></span></div>
                            <div><span class="text-gray-400 text-xs">Cota:</span>  <span class="text-gray-700 font-medium"><?= htmlspecialchars($sale['cota']  ?? '—', ENT_QUOTES, 'UTF-8') ?></span></div>
                            <div><span class="text-gray-400 text-xs">Tipo:</span>  <span class="text-gray-700 font-medium"><?= htmlspecialchars($sale['tipo'],          ENT_QUOTES, 'UTF-8') ?></span></div>
                            <div><span class="text-gray-400 text-xs">Crédito:</span> <span class="text-green-700 font-bold">R$ <?= number_format($sale['credito_contratado'] ?? 0, 2, ',', '.') ?></span></div>
                        </div>
                        <button type="button"
            ...
            <!-- Card: Pagamentos (PAG-01) — visível somente para Venda Fechada -->
                ...
                <div id="pagamentos-list" class="divide-y divide-gray-50">
                    ...
                    <div class="px-5 py-4 flex items-center justify-between gap-3" data-sale-id="<?= $sale['id'] ?>">
                        <div class="text-sm">
                            ...
                            <span class="text-gray-700 font-medium"><?= htmlspecialchars($sale['cota'] ?? '—', ENT_QUOTES, 'UTF-8') ?></span>
                            <span class="text-gray-400 mx-1">·</span>
                            <span class="text-gray-500 text-xs">Tipo:</span>
                            <span class="text-gray-700 font-medium"><?= htmlspecialchars($sale['tipo'], ENT_QUOTES, 'UTF-8') ?></span>
                        </div>
                        <?php if ($sale['is_paid']): ?>
                        <button type="button"
        ...
        <div class="bg-white rounded-xl shadow-xl p-6 max-w-md w-full mx-4">
            ...
            <div class="space-y-3">
                ...
                <div>
                    ...
                    <select id="cota-tipo" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:outline-none">
    ...
    (function() {
        ...
        // Delegação de evento: marcar cota como paga (PAG-02)
